...
public class SteeringPipeline : MonoBehaviour
{
    ...
    public void OnDrawGizmos()
    {
        if (this.draw)
        {
            ...
            else if (this.currentSmoothedSolution != null && CharacterInUse == CharacterType.Human)
            {
                HumanActuator humanActuator = (HumanActuator)Actuator;

                Gizmos.color = Color.green;
                if (humanActuator.FollowPathMovement.Target != null)
                    Gizmos.DrawCube(humanActuator.FollowPathMovement.Target.position, new Vector3(2f, 2f, 2f));

                Gizmos.color = Color.red;
                if (humanActuator.SeekMovement.Target != null)
                    Gizmos.DrawCube(humanActuator.SeekMovement.Target.position, new Vector3(2f, 2f, 2f));
            }
        }
    }
    ...
    private void Update()
    {
        ...
        Debug.DrawLine(this.character.KinematicData.position, actuatorPath.LocalPaths[0].EndPosition, Color.cyan);
